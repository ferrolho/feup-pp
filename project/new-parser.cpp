	...
	*/
	for (auto it = tissueSamplesMap.begin(), end = tissueSamplesMap.end(); it != end; it = tissueSamplesMap.upper_bound(it->first)) {
		...
		for (auto it = samplesRange.first; it != samplesRange.second;) {
			if (distance(samplesRange.first, it) != 1323) {
				auto nextIt = it;
				cout << distance(samplesRange.first, it) << " :: " << it->second << " - " << (++nextIt)->second << endl;
				outputStream << it->second << (++it != samplesRange.second ? '\t' : '\n');
			} else it++;
		}

		*tissuesFilesMap[tissueName] << outputStream.str();
		tissuesFilesMap[tissueName]->close();
	...
	for (unsigned int i = 0; i < gtexAnalysisFile.genesNum; i++) {
		...
		for (auto pair : tissuesFilesMap) {
			...
			for (auto it = samplesRange.first; it != samplesRange.second;) {
				...
				outputStream << (++it != samplesRange.second ? "\t" : "\n");
			}
